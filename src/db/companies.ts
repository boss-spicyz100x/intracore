...
export async function listCompanies(db: AnyDB): Promise<CompanyEntity[]> {
  const rows = await db.select().from(companies).where(isNull(companies.deletedAt));
  return rows;
}

export async function getCompanyById(db: AnyDB, id: string): Promise<CompanyEntity | null> {
  const [r] = await db
    .select()
    .from(companies)
...
export async function createCompany(db: AnyDB, input: CreateCompanyInput): Promise<CompanyEntity> {
